...
- Add **Environment Variables**:
  ...
  | `IMAGE_DAILY_LIMIT` | *(optional)* max AI images per day (default **40**). Each photo uses **one** image (a 6-sticker sheet, sliced locally); backgrounds use one each. Stored in `usage/image-<date>.json`, resets at UTC midnight. Set to `0` to disable the cap. |
